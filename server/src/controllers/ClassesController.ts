...
export default class ClassesController {
    ...
    async index(req: Request, res: Response) {
        ...
        if (!filters.week_day || !filters.subject || !filters.time) return res.status(400).json({ error: "Missing filters to search classes" })
    ...
    async create(req: Request, res: Response) {
        const { name, bio, avatar, whatsapp, cost, schedule, subject } = req.body
